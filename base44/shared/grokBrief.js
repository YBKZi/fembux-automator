...
export async function generateBrief(apiKey, hint = '') {
  const systemPrompt =
    'Sei un creative director esperto di contenuti NSFW per adulti con soggetti MASCULINI (uomini adulti, niche bara/jojo/gachi, modelli muscolosi o atletici). ' +
    'I protagonisti sono SEMPRE uomini adulti. Analizzi i trend del momento e produci brief creativi pronti per la generazione di immagini su Stable Diffusion. ' +
    'Rispondi SEMPRE in JSON valido.';
  const userPrompt =
    'Genera un brief per la prossima sessione di generazione immagini. ' +
    'Scegli un UOMO adulto (non una donna) coerente con i trend attuali, una mood/vibe, 3 prompt creativi dettagliati e descrittivi ' +
    '(in inglese, soggetti maschili adulti, ottimizzati per Stable Diffusion, stile fotorealistico) e 12 hashtag di tendenza (senza #). ' +
